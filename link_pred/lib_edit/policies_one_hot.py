...
action_qty_df = pd.read_csv('/Users/potato/Documents/IL/imitategrid/data/link_actions.csv')

# Create a dictionary that maps link_id to action_qty
action_qty_dict = pd.Series(action_qty_df.action_qty.values, index=action_qty_df.link_id).to_dict()
print('action_qty_dict',action_qty_dict)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        # Added mask
        self.action_qty_dict = action_qty_dict
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        log_prob = distribution.log_prob(actions)  #original
    ...
    # Added mask
    def create_action_mask(self, link_ids: th.Tensor, action_qty_dict: Dict[int, int]) -> th.Tensor:
        mask = th.full((len(link_ids), self.action_space.n), float('-1e6')).to(self.device) # Change -inf to -1e4
        for idx, link_id in enumerate(link_ids):
            # valid_action_count = action_qty_dict.get(link_id.item(), 1) + 1 # Always include 'stop' action
            valid_action_count = action_qty_dict.get(link_id, 1) + 1 # Always include 'stop' action
            mask[idx, :valid_action_count] = 0  # Unmask valid actions
        return mask

    # Create an instance of the OneHotEncoder
    def decode_one_hot_to_link_ids(self, link_ids_encoded, num_dimensions=427):
        id_list = []
        for i,link_id_encoded in enumerate(link_ids_encoded):
            link_id = np.where(link_id_encoded == 1.0)
            id_list.append(int(link_id[0]))
        return id_list
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor, obs: PyTorchObs) -> Distribution:
        ...
        # Added mask
        # link_ids = obs[:, 0].long()
        link_ids_encoded = obs[:, 0:427]
        link_ids_decode = self.decode_one_hot_to_link_ids(link_ids_encoded)
        # print('link_ids_decode',link_ids_decode)
        # print('link_ids_decode',len(link_ids_decode))
        mask = self.create_action_mask(link_ids_decode, self.action_qty_dict)
        mean_actions = mean_actions + mask
