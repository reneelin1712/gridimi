...
for state_traj, action_traj in zip(states, actions):
    ...
    distances = []
    directions = []
    for link_id_list_format in link_ids:
        link_id = link_id_list_format[0]
        if link_id in link_to_node_coords:
            node_coords = link_to_node_coords[link_id]
            distance = haversine(node_coords, last_step_node_coords)
            direction = calculate_direction(node_coords, last_step_node_coords)
        else:
            distance = 0.0
            direction = 0.0
   ...
   # Create a column vector filled with this link ID, with the same number of rows as `state_traj`
    ...
    concated_array = np.hstack((state_traj, speeds,one_hot_roadtypes,distances, directions,time_ranges, action_traj))
